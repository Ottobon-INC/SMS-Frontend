...
import { AlertCircle, Loader2, Building2, ArrowRight, CheckCircle2, Clock, FileText, Search } from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-16 text-slate-500 bg-white rounded-2xl border border-slate-200">
        ...
        <p className="text-sm font-medium">Loading institution overview…</p>
      </div>
    );
  }
  ...
  if (!sessions || sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-16 bg-white rounded-2xl border border-slate-200 text-center">
        ...
        <h3 className="text-lg font-bold text-slate-900 mb-1">No Attendance Data</h3>
        <p className="text-slate-500 text-sm max-w-sm">
          No attendance sessions have been recorded across the institution yet.
        </p>
      </div>
    );
  }
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
          </div>
              ...
              >
                <div className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-slate-500 text-xs">{session.batchName}</span>
                  ...
                  <button
                    ...
                    className="flex-shrink-0 flex items-center gap-2 px-4 py-2 bg-white hover:bg-slate-900 hover:text-white border border-slate-200 text-slate-700 text-sm font-semibold rounded-xl transition-all group-hover:border-slate-900 active:scale-[0.98]"
                  >
                    View
