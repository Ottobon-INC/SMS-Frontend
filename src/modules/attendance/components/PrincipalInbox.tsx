...
export const PrincipalInbox: React.FC = () => {
  ...
  return (
    <div className="space-y-10">
      ...
      {/* 2. Recently Finalized Section */}
      {finalizedSessions.length > 0 && (
        <section className="space-y-4 opacity-90 hover:opacity-100 transition-opacity">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Recently Finalized</h2>
            <span className="text-xs font-semibold text-slate-400">History</span>
          </div>

          <div className="space-y-3">
            {finalizedSessions.map(session => renderSessionCard(session, true))}
          </div>
        </section>
      )}
