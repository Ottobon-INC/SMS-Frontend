...
export const BranchesPage: React.FC = () => {
  ...
  const [savingProgrammes, setSavingProgrammes] = useState<boolean>(false);
  ...
  const handleOpenProgrammesModal = async (branch: Branch) => {
    ...
    try {
      const [_, branchRes] = await Promise.all([
        allMasterProgrammes.length > 0 ? Promise.resolve(allMasterProgrammes) : fetchMasterProgrammes(),
        fetch(`/api/v1/branches/${branch.id}/programmes`),
      ]);
  ...
  const handleSaveProgrammes = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!programmesBranch) return;

    setSavingProgrammes(true);
    try {
      const res = await fetch(`/api/v1/branches/${programmesBranch.id}/programmes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ programme_ids: assignedProgIds }),
      });

      if (res.ok) {
        setShowProgrammesModal(false);
        setNotification(`Offered programmes updated for ${programmesBranch.name}! Batches for 1st Year & 2nd Year created.`);
        setTimeout(() => setNotification(null), 4000);
      }
    } catch (err) {
      console.error('Failed to save branch programmes:', err);
    } finally {
      setSavingProgrammes(false);
    }
  };
