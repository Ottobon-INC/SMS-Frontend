  ...
  return (
    <div className="mx-auto max-w-7xl pb-12">
      ...
      <section id="overview" className="mb-4 sm:mb-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-xs md:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-wide text-teal-700">Dean / Institution Admin</p>
              <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-950 sm:text-3xl md:text-4xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3.5 py-2.5 text-xs text-slate-600">
              <div>
                <p className="font-bold text-slate-900">{auth.appUser?.display_name ?? "Dean"}</p>
                <p className="text-[10.5px] text-slate-500 mt-0.5">{todayLabel}</p>
              </div>
              <button
                type="button"
                onClick={() => void loadDashboard()}
                className="inline-flex items-center gap-1 rounded-lg bg-slate-200 px-2.5 py-1.5 text-[10px] font-bold text-slate-700 transition-colors hover:bg-slate-300 active:scale-95 cursor-pointer shrink-0"
              >
